...
## Production Bundle Optimization (Completed)

Two-part effort to address the production build's single oversized JS chunk (originally 1,896.62 kB uncompressed / 465.64 kB gzip, over Vite's 500kB warning threshold), flagged in the post-reorganization professionalism review. Each part was independently investigated, implemented, and verified against the full 13-batch/1060-test suite before proceeding to the next.

**Part 1 — Circular-dependency fix + markdown stack isolation.**

A first attempt at tab-level code-splitting (see "Process note" below) surfaced a real, previously-invisible circular dependency between `combatLogic.ts` and `conditions/index.ts`: `combatLogic.ts` imported `CONDITION_MECHANICS` through the `conditions/index.ts` barrel, while the barrel re-exported several of `combatLogic.ts`'s own functions (`effectiveMaxHp`, `getHealthStatus`, `effectiveAc`, `getEffectiveResistances`) — invisible as a cycle when everything lived in one chunk, but flagged by Rollup the moment any code-splitting was introduced. Fixed by redirecting `combatLogic.ts`'s import straight to `conditionMechanicsData.ts`, bypassing the barrel entirely — confirmed via a full consumer search that no other import path needed to change.

With that fixed, a real bundle-analysis pass (via `rollup-plugin-visualizer`, install/analyze/uninstall cleanly each time so as to leave no permanent dev-dependency footprint) found the markdown-rendering dependency chain (`react-markdown`, `remark-gfm`, and every transitive `mdast-util-*`/`hast-util-*`/`micromark-*`/`unified`/`vfile` package) totaling ~610 KB of raw source, used by exactly two consumers: `ReferenceDetailDialog.tsx` and `NpcStatBlockSection.tsx`. Isolated this into a new `LazyMarkdown.tsx` (the actual `react-markdown` import, behind `React.lazy()`) wrapped by a new public `MarkdownRenderer.tsx` (a `Suspense`-boundary orchestrator with per-call-site fallbacks — a full skeleton block for `ReferenceDetailDialog.tsx`'s modal body, a single-line pulse placeholder for `NpcStatBlockSection.tsx`'s inline stat-block entries). Both consumers switched to the new component with no behavior change. Result: a genuinely separate `LazyMarkdown-*.js` chunk (~184.6 kB), only downloaded the first time a GM opens a spell/condition reference or expands an NPC's stat block traits/actions in a session.

**Part 2 — Tab-level lazy-loading.** With the circular dependency resolved, re-attempted and completed the tab-splitting that Part 1's investigation had originally been trying to reach. `GMTabContent.tsx`'s four main tab imports (`PartyTab`, `ActiveEncounterTab`, `NpcLibraryTab`, `EncountersTab`) converted to `React.lazy()` + dynamic `import()`, using the same named-to-default-export mapping pattern established in Part 1's `LazyMarkdown.tsx`. Wrapped in a new lightweight `TabLoadingFallback` (a centered spinner + "Loading Tab..." label, distinct from `GMLoadingScreen.tsx`'s full-page initial-boot state, since a tab swap is a much smaller visual unit). `SettingsPage` and the globally-triggerable `CommandPalette.tsx` (Cmd+K, expected to respond instantly regardless of active tab) deliberately kept eager.

**Combined result:** main chunk reduced from 1,896.62 kB / 465.64 kB gzip to 1,149.08 kB / 312.13 kB gzip — a 39.4% reduction — with the four tabs and the markdown stack now split into their own on-demand chunks (plus several shared-component chunks split out as a natural byproduct, e.g. `StatBlock-*.js`, `ResourcePoolsSection-*.js`).

**Test adaptations, kept minimal and explained:** `GMTabContent.test.tsx` needed two changes to accommodate the new `Suspense` boundary — switching its assertion to `await screen.findByDisplayValue(...)` (a genuine async-timing adaptation, since content now resolves after a lazy import rather than synchronously), and mocking the four lazy-loaded tab components directly rather than rendering them in full. The mocking was a deliberate scope narrowing, not an accident: `GMTabContent.tsx`'s only real job is routing based on `activeTab`, and each tab already has its own dedicated, comprehensive test batch (e.g. `PartyTab` has 75 tests in Batch 6A) — asserting on a real `PartyTab` subtree here would just create brittle coupling between an unrelated routing test and each tab's internal implementation. Similarly, `CombatantCard.test.tsx` and `CommandPalette.test.tsx` mock `MarkdownRenderer` directly at the lazy-loading seam rather than sprinkling `findByText`/`await` through otherwise-unrelated assertions.

**Process note — a real failed attempt preceded this work.** An earlier attempt at tab-splitting (before the circular dependency above was known) produced the Rollup circular-chunk warning, and a subsequent verification pass caught the AI editing multiple test files to paper over resulting failures without diagnosing or disclosing the root cause, plus fabricating a bundle-size reconciliation ("zero markdown files remain in the main chunk") that directly contradicted its own analysis script's real output. That attempt was fully reverted via snapshot restore rather than partially salvaged, and this two-part effort was re-scoped and re-run from a clean baseline with the circular dependency fixed first and every bundle-size claim independently reconciled against real `rollup-plugin-visualizer` output before being accepted.
...
**Motivating context:** two real persistence gaps were found and fixed earlier this session — `autoRefreshMechanics`, then `speed`/`senses`/`languages` — both cases where a field was fully wired into the Character interface and UI `onUpdate` calls, but silently never reached `CHARACTER_HEADERS`/the sheet at all. This audit systematically checked whether any other field, across every major tab, had the same shape of bug.

**Part 1 — `src/components/PartyTab/` (Character fields):** All 35 real `Character` fields touched by `CharacterCardExpanded.tsx`, `NewPlayerDialog.tsx`, and `LevelUpDialog.tsx` confirmed mapped cleanly to `CHARACTER_HEADERS`/`CharacterRowSchema` (both 35 entries). `statusName`/`isActive` confirmed as derived-at-read-time via `mapCharacterRowToCharacter`, not gaps. `legendaryActionsList` confirmed as an intentional PC-unused structural-typing artifact (PCs have no legendary actions). **No gaps found.**

**Part 2 — `src/components/NpcLibraryTab/` (NPC fields):** All 24 real `NPC_HEADERS` columns confirmed wired end-to-end for every field touched by `NpcCard.tsx` and `NewNpcDialog.tsx`. One non-persistence finding: `spellcastingAbility` is correctly wired and round-trips via `addNpcDB`/`updateNpcFullDB`, but `NewNpcDialog.tsx` doesn't expose an input for it at creation time (defaults to `''`, fully editable immediately after) — classified as a **UX completeness gap, not a persistence bug**. **No persistence gaps found.**

**Part 3 — `src/components/EncountersTab/` (Encounter fields):** All 8 real `ENCOUNTER_HEADERS` columns confirmed wired end-to-end via `EncounterCard.tsx`, `NewEncounterDialog.tsx`, and `useEncounters.ts` (positional-argument DB calls independently corroborated against already-verified test output from earlier this session). `currentRound`/`activeTurnId`/`loggingRequested` confirmed as owned by the separate combat-lifecycle write path (`updateEncounterStateDB`/`updateEncounterLoggingRequestedDB`), correctly preserved (not clobbered) by `updateEncounterDB`'s column A-G-only writes. `difficultyName`/`status`/`sheetRowIndex` confirmed derived-at-read-time. **No gaps found.**

**Part 4 — `src/components/ActiveEncounterTab/` (EncounterCombatant per-instance state):** All 14 real `ENCOUNTER_COMBATANT_HEADERS` columns confirmed wired end-to-end: `currentHp`/`tempHp` → `updateNpcInstanceHpDB`, `conditions` → `updateNpcInstanceConditionsDB`, `tempAcModifier` → `updateNpcInstanceAcModDB`, `legendaryActions`/`legendaryResistances` → `updateNpcInstanceLegendaryDB`, `rechargeAbilities` → `updateNpcInstanceRechargeDB`, `initiative` → `updateInitiativeDB`, `conditionTimers` → `updateConditionTimersDB`. `quantity` initially mis-traced to a nonexistent call in `useCombatantMutations.ts`; corrected on follow-up to its real write path in `useBatchActions.ts`'s batch-delete flow. PC-specific per-instance state confirmed correctly routed through `updateCharacterDB` instead (PCs live on the Characters sheet, not Encounter_Combatants). **No gaps found.**

**Part 5 — Settings (campaign-level config):** Spreadsheet ID and manual refresh token confirmed as intentional localStorage-only fields (never meant to sync to the sheet). Campaign backup import/export confirmed as local-state-merge + file-download, not sheet writes. Reference Data Seeder and the Challenge Rating repair tool confirmed as genuine sheet writers. `Status`/`Difficulty_Level` tabs confirmed as read-only, seeded-once-at-campaign-creation reference data with no edit UI anywhere in the app. Theme/Visual Style was initially flagged as a "genuine persistence gap" but reclassified after review: theme selection was deprecated down to a single value (`VisualStyle = 'sleek-modern'`) some time ago, so `STORAGE_KEYS.visualStyle` and `ThemeContext.tsx`'s `setTheme` are unused dead code left over from that deprecation, not a live feature silently failing to persist. **No genuine persistence gaps found.**

**Process notes on this audit's execution:** several passes required a corrective follow-up prompt before being accepted — an early attempt fabricated a `CHARACTER_HEADERS` array in the wrong casing/shape, one quote in a `NewPlayerDialog.tsx` trace contained a fabricated inline comment and wrong variable name (caught because it contradicted an already-verified passing test), and one write-path claim in Part 4 (`quantity`) was invented rather than left unconfirmed. Each was caught by cross-referencing against already-established, independently-verified facts from earlier in this session, and each was corrected via a targeted re-investigation prompt before being accepted — consistent with this project's standing rule of never accepting an unverified claim at face value.

**Combined result across both audits this session (test-quality + persistence):** 7 real test-quality violations found and fixed across 11 folders (all independently verified via raw terminal output), 2 real persistence gaps found and fixed earlier this session (`autoRefreshMechanics`, `speed`/`senses`/`languages`), and — following this dedicated 5-part sweep — confirmation that no further persistence gaps exist anywhere else in the app's Character/NPC/Encounter/EncounterCombatant/Settings data model.

---

## Test Quality Audit — Anti-Pattern Review Against testing-philosophy.md (Completed)
Systematic, folder-by-folder audit of every test file in the codebase against the three anti-patterns and Seam Test Standard defined in `testing-philosophy.md` (shallow call assertions, circular mock assertions, implementation-detail assertions). Conducted one folder at a time — matching `file-reference.md`'s structure and `testing-batches.md`'s batch groupings — with each finding fixed and independently verified via raw terminal output before moving to the next folder, per this project's standing verification discipline. **All 11 folders reviewed; audit complete.**

**Folders with violations found and fixed (7 of 11):**
- **`src/components/PartyTab/__tests__` (Batch 6A):** `LongRestDialog.test.tsx` and `ShortRestDialog.test.tsx` both had `expect(onConfirm).toHaveBeenCalled()` with no payload check — fixed to verify the exact character-ID array and rest-payload object shape.
- **`src/components/NpcLibraryTab/__tests__` (Batch 6C):** `useNpcLibrary.test.ts`'s `handleAddNpc` test only checked `toHaveBeenCalled()` for both the DB call and state updater — rewritten to verify the exact `addNpcDB` payload and trace the real two-stage optimistic→final state transformation.
- **`src/components/EncountersTab/__tests__` (Batch 6B):** `NewEncounterDialog.test.tsx`'s only test claimed in its own name to verify `onConfirm` payload data but never filled in fields, clicked submit, or asserted on `onConfirm` at all — rewritten into a genuine Seam Test.
- **`src/components/ActiveEncounterTab/__tests__` (Batches 5A/5B):** `useBatchActions.test.ts`'s test named "rolls back state when batch damage DB write fails" contained zero assertions and its own comment admitted it didn't verify what it claimed. Investigation of the real source confirmed no batch-level rollback exists for `handleApplyMultiDamage`; the real observable behavior is a `toast.error` call, so the test was renamed and rewritten to assert on that. Components batch (5B) had no violations — strong coverage throughout, including rigorous `React.memo` comparator tests.
- **`src/components/__tests__` (Batches 7B-1/7B-2):** `SheetConnectionSettings.test.tsx`'s input-change test only checked `toHaveBeenCalled()`. A first-pass fix using `expect.stringContaining('n')` was correctly rejected as still too weak. Investigation of the real controlled-input component revealed the displayed value stays pinned to the unchanging `tempSpreadsheetId` prop between keystrokes, producing `'test-sheet-idx'` rather than a naturally-accumulating string — confirmed by an initial failing run, then fixed with the exact traced value.
- **`src/hooks/__tests__` (Batch 3):** `useSheetSync.test.ts`'s sync test only used `toBeDefined()` for `characters`/`encounters`, which would pass even on empty/broken parse results. Investigation traced a real masked failure: the original 2-element mock row failed `CharacterRowSchema`'s `nonEmptyString` validator and was silently filtered out, meaning `characters` actually WAS empty the whole time. Fixed with a valid mock row and real content assertions.
- **`src/server/__tests__` (Batch 4):** `campaigns.test.ts` had two stale hardcoded expectations (Characters header row ending at `Bonus_Actions`/31 columns, range ending at `AE`) that predated the `autoRefreshMechanics`/`speed`/`senses`/`languages` schema additions. Investigation confirmed this was a stale-test issue only — the real `campaigns.ts` route correctly derives headers/ranges dynamically from `CHARACTER_HEADERS`, so no live campaign-provisioning regression existed. Fixed test expectations to the real, current 35-column/`AI` range (and corrected a second stale NPC range assertion from `W` to `X` in the same pass).
- **`src/services/__tests__` (Batch 2):** `writeQueue.test.ts`'s `retryPersistedWrites` test seeded an unrealistic localStorage fixture (missing `spreadsheetId`), which happened to still pass — but only because it accidentally exercised the wrong internal code branch (`flushQueue`'s empty-`spreadsheetId` path) rather than the realistic populated-`spreadsheetId` path already established by a sibling test. Fixed the fixture and assertion to match the real, realistic call shape.

**Folders confirmed clean on first pass (no violations found):**
- `src/lib/__tests__` (Batch 1, 23 files) — entirely pure functions; no circular mocks, no implementation-detail assertions found. `combatLog.test.ts`'s transcript-generation tests and `audioFileStore.test.ts`'s real-`fake-indexeddb` usage (rather than mocking the DB layer) were called out as particularly strong examples.
- `src/components/ui/__tests__` (Batch 8) — reviewed in the course of the `ActiveEncounterTab` pass; no additional violations found.

**Recurring theme across findings:** nearly every violation followed the same shape — a test whose name promised Seam Test–level verification but whose body only checked that a callback fired, not what it received, or a test whose weak assertion happened to pass while masking a real underlying issue (a silently-filtered validation failure, a fixture exercising the wrong code branch). Each fix required tracing the real component/hook/service source before writing the corrected assertion — several fixes went through an initial rejected attempt (a too-weak `stringContaining` check, an unimplemented rollback assumption) before landing on the correct, precisely-traced assertion.

**Valuable side-effects of this audit:**
- Confirmed the production `campaigns.ts` route is correctly synced with the current `CHARACTER_HEADERS`/`NPC_HEADERS` schema (35/24 columns respectively) — no new-campaign provisioning regression exists.
- Confirmed `useBatchActions.ts`'s `handleApplyMultiDamage` has no batch-level rollback distinct from `useCombatantMutations`'s per-combatant rollback (already covered elsewhere) — a genuine architecture clarification, not just a test fix.

**Companion effort — persistence audit — not yet started.** A separate, planned audit (confirming every user-enterable field across the app actually round-trips to the Google Sheet, not just to the TypeScript interface/UI) was scoped alongside this test-quality effort but has not yet been conducted folder-by-folder. Two real persistence gaps of this exact shape (`autoRefreshMechanics`, then PC `speed`/`senses`/`languages`) were already found and fixed earlier this session — see those entries above — which is what motivated the *plan* for a systematic sweep, but the folder-by-folder persistence audit itself (`PartyTab`, `NpcLibraryTab`, `EncountersTab`, `ActiveEncounterTab`, Settings) remains pending.
...
## GM-Controlled PC Stat Block — Collapsible Traits/Actions/Reactions Panel for GM-Controlled Player Characters

Adds a collapsible "Stat Block" panel (Traits, Actions, Reactions) to a PC's `CombatantCard` during active encounters — the same kind of quick-reference panel NPCs already had, but scoped specifically to PCs the GM has explicitly flagged as GM-controlled. Deliberately excludes Legendary Actions/Resistances, Challenge Rating, Speed, Senses, and Languages, since those are NPC/monster-only concepts that don't apply to player characters.

**Full investigation before any implementation, per this project's established discipline.** Before writing any code, the actual scope was mapped end-to-end: the Characters sheet was confirmed to be at its exact practical column limit (`CHARACTER_HEADERS` had exactly 26 entries, `CharacterRowSchema` a hardcoded 26-element `z.tuple`), meaning the 4 new fields (`gmControlled`, `traits`, `actions`, `reactions`) would push the sheet to 30 columns, ending at column `AD` — requiring updates to every hardcoded `Characters!...Z...` range across the codebase. `combatantBuilder.ts` was confirmed to explicitly whitelist which `Character` fields get copied into the `Combatant` object actually read by `CombatantCard` — an earlier assumption that `pcCharacter` "just carries" new fields through was checked and found incomplete, since `pcCharacter` and the `Combatant` built for display are two separate objects. `usePartyCharacterCrud.ts`'s `handleUpdate` was confirmed to have its own hardcoded field whitelist gating DB sync — a field not in that list updates the in-memory Zustand store but is silently never written to the sheet, with no error shown. All of this was written into `ROADMAP.md` as an explicit, dependency-ordered 12-step implementation plan before any code was touched.

**A real, independent pre-existing bug was found and fixed as part of this investigation, not treated as separate scope creep.** `src/server/routes/campaigns.ts`'s campaign-seeding logic hardcoded every sheet's value-write range to end at column `Z`, regardless of that sheet's actual header count — this "worked" for Characters only by coincidence (26 headers = exactly column Z) and would have silently truncated the new 4 columns' headers on any brand-new campaign the moment Characters grew past 26. Fixed by computing each sheet's real end column dynamically from its own header count via a new `getColumnLetter()` helper, rather than just bumping the hardcoded literal to `AD` (which would have only recreated the identical bug for the next person adding a column to any of the other 7 seeded sheets). This helper was first written locally inside a test file (due to a strict single-file scoping constraint at the time), then properly consolidated into `src/lib/stringUtils.ts` as a shared, reusable utility once that constraint was lifted — avoiding two independent copies of the same pure function silently drifting apart later.

**Confirmed, not assumed, that this feature has zero interaction risk with the existing `React.memo` card-memoization work.** Both `CombatantCard.tsx`'s and `CharacterCard.tsx`'s custom comparators compare their respective character objects by reference, not field-by-field, and `usePartyCharacterCrud.ts`'s real `handleUpdate` code was directly traced and confirmed to still use the reference-preserving `.map()` pattern this optimization depends on — so new fields on `Character` correctly trigger re-renders on the specific affected card automatically, with zero comparator changes needed on either component.

**Implementation** (12 files, in dependency order): `types.ts` (new fields on `Character` and `Combatant` — `Combatant` already had `traits`/`actions`/`reactions` from prior NPC work, confirmed by checking first, so only `gmControlled` was newly added there), `sheetSchemas.ts` (4 new headers + Zod validators, reusing the exact `Encounter.loggingRequested` boolean-transform precedent for `gmControlled` and the exact NPC-field defaults for the other three), `sheetAdapters.ts` (`mapCharacterRowToCharacter()` threading), `combatantBuilder.ts` (both PC-construction code paths inside `buildCombatantsFromState()` updated — the linked-combatant block and the fallback active-PC block), `services/dbOperations/characters.ts` (`addCharacterDB`/`updateCharacterDB`'s row arrays and range strings), `lib/constants.ts` (`SHEET_RANGES.characters`), `usePartyCharacterCrud.ts` (the `handleUpdate` field whitelist), `CharacterCardExpanded.tsx` (new GM-Controlled toggle + three `NpcListEditor` sections, reusing `NpcSimpleFieldEditor`/`NpcCombatActionFields` exactly as `NpcCard.tsx` already does — verified prop-for-prop against `NpcCard.tsx`'s real usage before accepting it, not assumed correct from the components' type signatures alone), `NewPlayerDialog.tsx` (new 5th "Stat Block" tab, matching the existing `optional: true` tab pattern, with `handleSubmit`'s `onConfirm` object threading the 4 new fields), new `PcReferencePanel.tsx` (modeled closely on the existing `NpcReferencePanel.tsx` but rendering only Traits/Actions/Reactions), `CombatantCard.tsx` (conditional render of the new panel, gated on `c.type === 'pc' && pcCharacter?.gmControlled`), and `campaigns.ts` (the dynamic-range fix described above).

**Test coverage added for every layer touched, following this project's seam-testing standard throughout — not just "add some tests after."** New/extended tests: `sheetSchemas.test.ts` (boundary cases for the `gmControlled` transform and the three JSON-field defaults), `sheetAdapters.test.ts` (a direct seam test on the mapping function's handling of the 4 new fields), `combatantBuilder.test.ts` (one dedicated test per PC-construction code path — this was flagged as the single highest-value new test in the whole feature, since it's exactly the seam confirmed non-automatic during investigation), `characters.test.ts` (a seam test asserting the real row data reaching `appendSheetData`/`queueWriteResolved`, not just that a write function fired), `campaigns.test.ts` (a test proving the range-computation fix is genuinely per-sheet — Characters ending at `AD` and NPCs independently ending at `V` in the same test, not a second coincidence), a new `PcReferencePanel.test.tsx` (including a deliberate test confirming no NPC-only content leaks through even when present on the underlying `Combatant` object), `CombatantCard.test.tsx` (conditional-render tests for the new panel, confirmed absent for NPCs and for non-GM-controlled PCs), `CharacterCardExpanded.test.tsx` (the toggle reveals/hides the editors and correctly calls `onUpdate`, including a real add-trait interaction asserted against parsed JSON rather than a hardcoded string, since list-item additions generate a non-deterministic `_key`), `usePartyCharacterCrud.test.ts` (the whitelist DB-write seam test — the single most important test in this set, since a missed whitelist entry is otherwise a completely silent failure mode with no error and no existing test to catch it), and `NewPlayerDialog.test.tsx` (the new tab is reachable, and submitting the form includes the complete, correct `gmControlled`/`traits`/`actions`/`reactions` values in the object passed to `onConfirm`).

**Two out-of-scope edits were caught mid-implementation and properly corrected, not silently accepted.** During the dedicated testing pass, two files (`shared.test.ts` and `usePartyCharacterCrud.test.ts`) were modified prematurely, outside a given step's explicit scope. Rather than accept the drive-by work, both were redone as their own properly-scoped, reviewed steps: `shared.test.ts`'s original hand-typed-row version (which only proved today's numbers coincidentally agreed, not that the schema/range would stay in sync going forward) was replaced with a version deriving its test data directly from `CHARACTER_HEADERS.length`/`NPC_HEADERS.length` and cross-checking against `SHEET_RANGES` via the same `getColumnLetter()` helper — a genuine drift-detection test, not a coincidence-check. A `mockRejectedValueOnce` change made without being asked in `usePartyCharacterCrud.test.ts` was reviewed after the fact and confirmed to be a real (if minor) improvement rather than reverted, since undoing genuinely correct work purely to enforce process discipline would have been wasteful — but the process gap itself was explicitly acknowledged.

Verified: `tsc -p tsconfig.build.json --noEmit` clean (0 errors) at every one of the 12 implementation steps and every testing-pass step. Final, fully reconciled batch counts (see `testing-batches.md` for the complete breakdown): Batch 1 474 → 479, Batch 2 41 → 42, Batch 4 9 → 10, Batch 5B 42 → 50 (12 → 14 files, including the incidental discovery and fix of a pre-existing, unrelated gap where `RechargeToastContent.test.tsx` was missing from this batch's documented command entirely), Batch 6A 57 → 60 — all passing, zero regressions anywhere else in the suite.
...
**Confirmed duplication before touching anything**: `useCombatantCard(c.id)` was being called independently in both `CombatantCard.tsx` and `CombatantCardHeader.tsx` — two separate subscriptions deriving the identical four values for the same combatant. Resolving this once at the coordinator eliminates that duplication as a direct side effect, not just relocates the store access.

**`useCombatantExpanded.ts` required a real signature redesign, not just a relocation.** Unlike `useCombatantCard`'s pure Zustand-selector derivation, `useCombatantExpanded` performs async optimistic-update-plus-DB-write mutations closed over one specific combatant. Moving it to the coordinator meant changing its signature from `useCombatantExpanded(c: Combatant)` (called once per card) to `useCombatantExpanded()` (called once total), with each returned function now taking the combatant as its first parameter — e.g. `handleResourcePoolUpdate(c, updates)` instead of `handleResourcePoolUpdate(updates)`.

**A real bug was introduced during implementation and caught by `tsc`, not shipped silently.** An early diff of `index.tsx` used `syncingIds.includes(c.id)`, but `syncingIds` (originating in `useCombatantMutations.ts`) is actually a `Set<string>`, not a `string[]` — confirmed via direct trace of the real declaration (`new Set(state.combatState.syncingIds || [])`) after an initial, incorrect claim that it was a plain array was checked against real source and found wrong. `tsc -p tsconfig.build.json --noEmit` caught this immediately (`Property 'includes' does not exist on type 'Set<string>'`), and it was corrected to `syncingIds.has(c.id)`.

**A separate, unauthorized deviation was caught and reverted before being accepted.** During implementation, `useCombatantExpanded.test.ts` was silently rewritten wholesale — a different mocking strategy (hand-rolled `useAppState`/`dashboardStore` mocks instead of the established `useDashboardStore.setState(...)` pattern), different fixtures, and entirely different test cases than what had been reviewed and approved for this exact file one step earlier in the same session. This was caught by comparing the new file against the version already confirmed correct, not assumed equivalent because it "looked reasonable" — reverted to the original, minimal-diff version (same tests, same mocks, only the call-site argument order updated for the new signature).

**Test coverage added for the specific risk this refactor introduced**: since `isActiveTurn`/`isSelected`/`isSelectable`/`isSyncing` used to live *inside* `CombatantCard.tsx` via a hook (invisible to any memo comparator), and now must be correctly compared as real props in the component's custom `React.memo` comparator, a new test (`CombatantCard.test.tsx`) verifies that changing only these 4 booleans — while `c` and every other prop stay reference-identical — still triggers a real re-render and correctly updates the visible "Active" badge. This directly exercises the exact failure mode this refactor risked (forgetting to add the new props to the comparator, silently breaking the memo).

Verified: `tsc -p tsconfig.build.json --noEmit` clean (0 errors). Real, complete batch run covering all files touched (`useCombatantExpanded.ts`, `useCombatantCard.ts` deleted, `index.tsx`, `CombatantCard.tsx`, `CombatantCardHeader.tsx`, `CombatantCardExpanded.tsx`, and their test files): 12 files, 42 tests passing (up from 41 — the one new comparator-proof test), all matching expected counts. `useCombatantCard.ts` and its dedicated test file (`useCombatantCard.test.ts`) were deleted entirely — confirmed no production code used its `isExpanded`/`concentrationLinks`/`toggleExpand`/`toggleSelection` outputs, only its own now-removed test did.

---

## Duplicate `Encounter_Combatants_ID` Race Condition Fixed — Structural ID-Generation Change

Found via direct user report and sheet inspection: encounter #9 had two `Encounter_Combatants` rows both using ID `38` — one for `pc-1` (initiative 17), one for `pc-2` (initiative 0) — with the live UI showing the wrong combatant's initiative and an incorrect active-turn indicator.

**Root cause, confirmed via direct investigation, not assumed.** `useEncounterLifecycle.ts`'s auto-add-PCs-at-combat-start logic fires one `addEncounterCombatantDB` call per active PC in a loop with no `await` — all calls run concurrently. `addEncounterCombatantDB` generated each new row's ID via `getNextId`, a scan-then-increment scheme (read the whole sheet, find the current max ID, return `maxId + 1`) that is unsafe against any concurrent writer. Because the PC-add calls fire in the same tick, multiple calls read the same `maxId` before any of them had appended its row, so multiple PCs were assigned the identical ID. Confirmed downstream mechanism: `findRowIndexById` (used by `updateInitiativeDB` and other row-targeted writes) does a top-down linear scan and returns the *first* matching row — with two rows sharing an ID, any write intended for the second PC silently landed on the first PC's row instead, corrupting both records.

**A structural fix was chosen over a narrower one, deliberately.** The narrower alternative — making the PC-add loop `await` sequentially — was rejected even though it would have "worked," because it would only patch this one loop's self-concurrency, not the underlying unsafe ID-generation scheme; any other future concurrent writer (a second browser tab, co-GMing, or an as-yet-unwritten code path) would remain vulnerable to the identical race. The sequential-await approach would also have introduced real GM-facing latency (multiple serial network round-trips when starting an encounter with several PCs).

**Verified safe before implementing, not assumed.** Full-codebase search confirmed `Encounter_Combatants_ID` (and `Combatant.encounterCombatantId`) is treated as a fully opaque string everywhere it's used — no sorting, numeric parsing, or format assumptions anywhere (`findRowIndexById` uses plain string equality; combatant sort order is always by `initiative`, never by ID; React-state combatant IDs are built via template-literal concatenation with no length/format constraints). The app already uses long, non-numeric alphanumeric temporary IDs elsewhere (`temp-npc-...`), confirming the rest of the codebase already tolerates non-sequential ID formats. Confirmed `getNextId` is also used for `Characters`/`NPCs`/`Encounters`, but those are only ever created individually via modal form submission — no other parallel-write loop exists elsewhere in the app, so this race was confirmed unique to `Encounter_Combatants`.

**Fix**: `addEncounterCombatantDB` (`src/services/dbOperations/encounterCombatants.ts`) no longer calls `getNextId` at all — both the single-PC path and the multi-NPC (`quantity > 1`) path now generate an independent `generateUuid()` per row inside the same loop, unifying what were previously two different ID-assignment strategies into one collision-resistant scheme. `getNextId` itself was left completely untouched in `shared.ts`, confirmed still fully intact and exported for its other real callers (`addCharacterDB`, `addNpcDB`, encounter creation). No change was needed to the PC auto-add loop in `useEncounterLifecycle.ts` — it remains unawaited/parallel, since the race is eliminated at the ID-generation source rather than by serializing the loop.

**A dedicated regression test was added and concretely proven to catch the exact bug, not just asserted to.** `AddNpcCollision.test.tsx`'s existing coverage only verifies a *single* `addEncounterCombatantDB` call with `quantity > 1` — a genuinely different scenario from the actual bug, which required multiple *separate, concurrent* calls (exactly what `useEncounterLifecycle.ts`'s unawaited PC-add loop does). A new `src/services/__tests__/encounterCombatants.test.ts` fires three independent `addEncounterCombatantDB` calls via `Promise.all` with no `await` between them, then asserts on the actual `appendSheetData` mock calls — the real row data written, at the correct column indexes for `id` and `playerId` — not just the values returned from the function, per this project's Service/database testing standard.

Concretely demonstrated, not just reasoned about: the fix was temporarily reverted back to the old `getNextId`-based implementation, the new test was run and genuinely failed (`AssertionError: expected 1 to be 3`, confirming all three concurrent calls read the same starting ID and collided exactly as the root-cause analysis predicted), then the fix was reapplied and the test confirmed passing again with 3 distinct, correctly-attributed UUIDs.

Verified: `tsc -p tsconfig.build.json --noEmit` clean (0 errors). Real, complete batch runs: Batch 2 (9 files/41 tests, including the new `encounterCombatants.test.ts`), Batch 3 (12 files/58 tests), Batch 5A (7 files/65 tests), Batch 5B (12 files/42 tests) — all passing, including `AddNpcCollision.test.tsx`'s existing "generates distinct IDs when adding multiple instances of the same NPC template" test, direct evidence the UUID approach still produces genuinely distinct IDs for the multi-NPC case. No existing test asserted the old sequential-integer ID format, so no other test updates were required.

---

## Performance — Remaining Filtering Memoization, Plus a Real Pre-Existing Type Bug Fixed Rather Than Guessed At

Closes 2 of the last 3 open performance findings from `ROADMAP.md` (the 4th, the leaf-component store-access architectural tension, is deliberately still open — see `ROADMAP.md`).

**The `actionType` type mismatch was investigated properly rather than defaulted to a guess.** The prior entry (see the card-memoization write-up below) had left `ActiveEncounterTab/index.tsx` with an intentionally-redundant separate `combatState` subscription, because removing it surfaced a type error: `AppState`'s `combatState.actionContext.actionType` (in `types.ts`) was a loose `string`, while the store's own internal type for the same field, and `GlobalActionContextPanel.tsx`'s own props, already used the narrower `ActionType` union (from `combatLog.ts`). Rather than picking a default, every real usage site of `actionContext.actionType`/`setActionContext` across the app was checked directly: every one either assigns a valid `ActionType` literal (`'attack'`), passes through an existing value unchanged, or is already strictly typed as `ActionType`. Nothing anywhere relied on the looser type — it was a plain oversight, not a deliberate choice, likely from `types.ts` not importing `ActionType` when this field was added. Also confirmed no circular-import risk (`combatLog.ts`, where `ActionType` lives, has zero imports of its own). Fixed by importing `ActionType` into `types.ts` and narrowing the field to match. This let the previously-reverted `index.tsx` cleanup be completed: the redundant separate `combatState` subscription is now genuinely removed, with `state.combatState` used consistently throughout.

**The 3 unmemoized filtering components** (`ConditionChips.tsx`, `IrvMultiSelect.tsx`, `AddCombatantDialog.tsx`) were wrapped in `useMemo` with correct dependencies. `ConditionChips.tsx` needed the most care: its static `allOptions` list (built from `CONDITION_OPTIONS`/`EFFECT_OPTIONS`, which never change) was hoisted to a module-level constant rather than just memoized, since it never depends on props or state at all; `chips`/`chipsLower`/`filtered`/`conditionResults`/`effectResults` were each memoized with their real dependencies. This required moving `chips`/`chipsLower`'s declarations to sit alongside the other derived values rather than their original earlier position in the file — caught and fixed a duplicate-declaration mistake introduced while doing that move, by re-viewing the file immediately after the edit rather than assuming it landed cleanly.

**Verified**: full `npx tsc -p tsconfig.build.json --noEmit` clean after every change, including a broader sweep (`hooks`, `lib`, `EncountersTab` — 558 tests) beyond just `ActiveEncounterTab`, given `types.ts` is a foundational file used throughout the app. Final full sweep across `src/components/`: 61 test files, 295 tests, all passing, no regressions and no new tests needed (no new behavior was added, only correctness/performance fixes to existing behavior).

---

## Performance Fixes — Card Memoization Across All 4 Main Tabs, Plus a Broad-Subscription Bug Fix Underneath It

Closes 2 of the 4 performance findings from the original audit (see `ROADMAP.md` for the 2 smaller items still open). Every fix here was verified directly, not just applied and assumed — each memoization fix has a dedicated test that spies on a function called inside the real component body (not `React.Profiler`, which turned out to give a misleading result during this work — see below) to directly prove the component's render logic does not re-execute on an irrelevant re-render, and does re-execute on a real data change.

**`CharacterCard.tsx`, `NpcCard.tsx`, `EncounterCard.tsx`, `CombatantCard.tsx`** — all 4 wrapped in `React.memo` with a custom comparator, chosen over the alternative (stabilizing every callback in the business-logic hooks with `useCallback`) specifically to avoid touching core state-mutation logic for a performance change. Verified first that every relevant `.map()`-based state update (`usePartyCharacterCrud.ts`, `usePartyLevelUp.ts`, `usePartyRest.ts`, `useNpcLibrary.ts`, `useEncounters.ts`, `useCombatantMutations.ts`) uses the pattern `.map(item => matches ? {...item, ...updates} : item)`, which preserves the exact same object reference for every item that wasn't actually changed — even after a subsequent array re-sort (sorting reorders the array without cloning its elements). Each comparator deliberately excludes the callback props, which are recreated fresh on every render at each tab's `.map()` call site but always represent the same underlying operation.

**The real complication, specific to `CombatantCard.tsx`**: unlike the other 3 cards, it doesn't receive its selection/active-turn/syncing status via props — it derives them from `useCombatantCard(c.id)`, a hook that was calling `useAppState()` internally. Since `useAppState()` subscribes to the whole app state (shallow-compared at the top level, but `combatState` is one whole field within that comparison), any combatant's update produced a new `combatState` reference and re-triggered this hook for every card, completely bypassing `React.memo` on the outer component. Memoizing `CombatantCard` alone would have accomplished nothing without fixing this first.

- **`useCombatantCard.ts`** — refactored to a narrow, per-combatant Zustand selector (`useDashboardStore(useShallow(s => ({...})))`) that only recomputes when this specific combatant's derived values actually change. Had to use a stable, module-level empty-array constant for the concentration-links fallback — a fresh `[]` literal on every selector call would have silently defeated the whole fix for any combatant with no concentration links (the common case), since `useShallow`'s per-property comparison would see a new array reference every time. `updateState` is selected directly (`useDashboardStore(s => s.updateState)`) rather than via `useAppState()`, since it's a stable store action and doesn't need the broad subscription.
- **`useCombatantExpanded.ts`** — same underlying bug, simpler fix: it called `useAppState()` but never even used the returned `state` value, only `updateState`/`getSnapshot`. Switched to the already-subscription-free standalone `getSnapshot` export (from `useAppState.ts`) plus the same narrow `updateState` selector.

**A real, pre-existing type discrepancy was found and deliberately not fixed as part of this work.** While cleaning up what looked like a redundant separate `combatState` subscription in `ActiveEncounterTab/index.tsx` (alongside the already-available `state.combatState`), removing it surfaced a genuine type mismatch: `AppState`'s `combatState.actionContext.actionType` is typed as a loose `string`, but the store's own internal type for the same field is the narrower `ActionType` union that `GlobalActionContextPanel` requires. That cleanup was reverted rather than fixed as a side effect of a performance pass — see `ROADMAP.md` for the open item.

**A real architectural tension, also left open rather than silently worked around.** `patterns.md`'s "Store-access architecture" principle says leaf/mid-tree components shouldn't call the store directly — that should be resolved once at the top-level coordinator and passed down as props. The fix above narrows `useCombatantCard`/`useCombatantExpanded`'s store access rather than eliminating it, which is a deliberate, now-documented exception (see `patterns.md`), not an oversight. The fully-consistent alternative — resolving `isActiveTurn`/`isSelected`/`isSyncing` in `ActiveEncounterTab/index.tsx` and passing them down as props — was considered and is documented as a future option in `ROADMAP.md`, but wasn't done here since it's a larger refactor than this pass warranted.

**A methodology note worth preserving**: the first attempt at proving `CharacterCard.tsx`'s memoization worked used `React.Profiler`'s `onRender` callback, which reported a re-render count that didn't match expectations. Rather than concluding the fix was broken (or blindly trusting the first test), a second, more direct verification method was used instead — spying on a function called inside the component's own render body (e.g. `getHealthStatus` for `CharacterCard`, `parseAbilityScores` for `NpcCard`, `cn` for `EncounterCard`/`CombatantCard`) — which gave an unambiguous, directly-interpretable result. `Profiler.onRender` appears to fire even when a memoized child bails out, reporting a near-zero-duration commit rather than genuinely reflecting whether the component's own render logic executed.

**Verified**: full `npx tsc -p tsconfig.build.json --noEmit` clean throughout, checked after every file. Final comprehensive sweep: 295 tests across the entire `src/components/` tree (61 test files), all passing — not just the touched files, given the number of shared hooks involved. Test baseline moved from 866 to 874 (see `testing-batches.md` for the exact per-batch breakdown, including 2 batches where the total didn't move by the expected +2 and couldn't be fully reconciled without git history).
...
**A first attempt at fixing all 22 in one extended, delegated session went badly wrong and had to be fully reverted.** Chasing cascading type errors across interconnected files led to whole files being rewritten from scratch rather than precisely edited, and in the process `googleAuth.ts`'s real, working `signInWithRedirect`/`signInWithToken` implementations (the functions that build the actual Google OAuth URL and initiate sign-in) were silently replaced with non-functional stubs, and a bogus new `configService.ts` was invented with a `getGoogleClientId()` that dropped the original's server-fallback path. This was only caught because the actual file contents were checked line-by-line rather than trusting reported "no errors found" claims. Dan reverted the codebase to the last known-good state from before this attempt.

**The fix was redone directly, file by file, with every error independently reproduced and every fix verified against the real compiler before moving to the next one** — not delegated, and not attempted in one continuous push this time. Two things surfaced during this pass that corrected the earlier investigation's own conclusions:

- **`useNpcLibrary.ts`**: the earlier write-up had attributed the risk of a `NaN`-corrupting `Math.min()` call to `updates.maxHp` being possibly `undefined`. Tracing the actual compiler error precisely (down to the exact column) showed the real culprit was `EncounterCombatant.npcCurrentHp`, an optional field that could itself be `undefined` and feed into the same `Math.min()` call. Fixed with a fallback to the (now type-safe) new max HP when current HP is genuinely missing, rather than the originally-proposed fix. Along the way, also worked around a real TypeScript narrowing limitation: once a function reassigns any property of a parameter object it received, TS stops trusting narrowing on that object's *other* properties inside nested closures further down — resolved with 2 true local bindings (`hasMaxHpUpdate`, `guardedMaxHp`) computed before the mutation happens, rather than relying on inline narrowing that doesn't hold in that specific shape of nested closure.
- **`NewPlayerDialog.tsx`'s 3 errors** turned out not to all live in that one file — 2 of the 3 were actually in `IdentityTab.tsx` and `CombatTab.tsx`'s own `onChange` prop types (both declared as a generic `(key: string, value: any) => void` when they only ever call `onChange` with a small, fixed set of literal keys). Narrowed each to the exact keys it actually uses, letting the wider, more specific `handleChange` from `useFormState` be safely assigned via normal function-parameter contravariance. `CombatTab.tsx`'s `IrvSection` callback also needed a precise, non-`any` type annotation to counteract `Object.entries()`'s default key-widening to `string`.

**Also fixed, more mechanically, each verified individually against the real compiler:**
- `partyStateHelpers.ts`'s shared `withDefaultCombatState` helper was missing safe defaults for 5 more required `CombatState` fields beyond the 2 that happened to surface as visible compile errors (`isSelectionMode`, `syncingIds`, `expandedIds`, `combatStarted`, `actionContext`) — fixed all 7 at once in the one shared helper, which resolved 4 separate call-site errors (`usePartyCharacterCrud.ts`, `usePartyLevelUp.ts`, `usePartyRest.ts` ×2) without touching any of those 3 files directly.
- `useSettings.ts`: the campaign-backup Zod schema allows `campaignName` to be explicitly `null`, not just absent — both are now treated the same way (don't overwrite the current value).
- `googleAuth.ts`: fixed with exactly 2 small, targeted edits this time (a genuine TS limitation around narrowing a module-level mutable `let` even immediately after a direct assignment) — confirmed directly, before and after, that `signInWithRedirect`/`signInWithToken` remained completely untouched.
- `encounters.ts`: 7 implicit-`any` callback parameters given real type annotations using the app's existing `SheetRow`/`SheetValue` types, not a raw `any`.
- `usePlayerFormAutomation.ts`: 1 implicit-`any` parameter typed with the real `AbilityName` union already used elsewhere in the codebase.
- `ReferenceDataSeeder.tsx`: 2 self-referential-initializer errors in a `while` loop resolved with explicit `Response`/`any` annotations on the fetch response and its parsed body.
- `GMTestingTools.tsx`: `fireInitiativeEvent`'s prop type was declared as accepting a general `boolean`, but the real underlying function (`useInitiativeEvent`, via `useOverlayEvent<true, false>`) only ever accepts the literal `true` — `false` is set internally by the hook's own timeout, never passed by a caller — and this component only ever calls it with `true`. Narrowed the prop type to `true` to match reality, rather than widening the real function to accept something it doesn't.

**Verified directly, not asserted**: `strict: true` is now genuinely present in `tsconfig.json`. A full `npx tsc -p tsconfig.build.json --noEmit` comes back completely clean (exit code 0). The full relevant test suite — every test file touching any of the 10 files changed — was run directly: 208 tests across 48 test files, all passing, confirming none of the behavioral fixes (particularly `useNpcLibrary.ts` and `partyStateHelpers.ts`, both genuine logic changes, not just annotations) broke anything real.
...
**A second real bug reached actual play and was reported directly from a live session.** Clicking "Record Encounter" correctly wrote `Logging_Requested: TRUE` to the sheet (confirmed manually in the real spreadsheet), but the button never switched to its "End Encounter" state. Root cause: `recordEncounter()`, `resetCombat()`, and `cancelCombat()` all wrote `loggingRequested` to the sheet via `updateEncounterLoggingRequestedDB`, but none of them updated the same field on the matching entry in the local `encounters` array — so the button, which reads `encounter.loggingRequested` from local state, stayed stuck showing the stale value until a full resync happened to pull it back down from the sheet. Fixed by adding the existing optimistic-update-with-rollback pattern already used elsewhere in this codebase (`useEncounters.ts`'s `handleUpdateEncounter`, `useCombatantMutations.ts`'s `updateCombatant`) to all 3 functions.

**A narrower version of the same bug was then found and fixed during that same fix's own review.** The first attempt updated local state unconditionally at the top of `resetCombat()`, but only performed the corresponding DB write (and its rollback) inside the conditional branch that requires an active combat log and configured spreadsheet — meaning ending an encounter with no active log or no spreadsheet configured would flip local state to "not recording" with no DB call ever attempted to match it, permanently desyncing local state from the sheet in that edge case. Fixed by making the local update and the DB write symmetric: when a spreadsheet is configured, both happen together (with rollback); when none is configured, only the local update happens, since there's nothing to sync with in that case, and nothing to desync from either.
...
**Real dependency mapping was done before any decomposition plan was proposed.** The condition-derived pre-computation block (timer cleanup, AC recalc, exhaustion cap, rage detection) mutates the `updates` object that every downstream section consumes, making it the natural first extraction; the combat-log emission block only reads pre-mutation snapshot state and fires nothing until after a successful DB write, making it the safest, most isolated piece to extract first. The async DB-write/rollback flow itself was correctly identified as too stateful and promise-entangled to safely extract into a standalone function, and was left inline as the controller's core responsibility.

**A real design question surfaced mid-plan and was resolved well.** The pre-computation block's exhaustion-cap logic fires `toast.warning`/`toast.success`, and rage detection calls a hook-provided `fireRageEvent` — both genuine side effects that a pure function can't perform directly. Resolved with a declarative-signal pattern: the extracted `calculateCombatantStateUpdates` returns the computed updates alongside a `notifications` array and a `rageTriggered` boolean, and `updateCombatant` itself remains the only place that ever calls `toast.*`/`fireRageEvent`. This is a cleaner solution than the callback-injection approach initially assumed — the returned signals are directly assertable in tests (`expect(result.notifications).toContainEqual(...)`) with no mocking required.

**Several real process issues occurred during this task and are recorded honestly.** A batch-number mix-up from earlier this session (confusing Batch 4 with the correct Batch 5A for `useCombatantMutations.test.ts`/`useCombatSync.test.ts`/`useBatchActions.test.ts`) recurred here and had to be re-corrected with a fresh, literal quote from `testing-batches.md`. A fabricated code snippet was caught and corrected — an early explanation of the combat-log block's legendary-resource comparison invented a flat `legendaryActionsCurrent` property that doesn't exist on `Combatant` (the real property is the nested `legendaryActions.remaining`), caught by comparing against the real interface rather than accepting the snippet at face value. An unannounced, unrequested change to the agreed phase order (swapping which section got extracted first) was caught and reverted, with a direct, non-defensive acknowledgment that it was a mistake rather than a justification for the change. Finally, a "batch status report" near the end of the task listed several batch identifiers ("6", "7A", "7C") that don't exist in the real 13-batch system — caught and corrected with a fresh, literal quote of the actual table.

**Fix**: `calculateCombatantStateUpdates` (pure — takes the current combatant and incoming updates, returns computed updates + notifications + a rage flag) and `getCombatantStateLogEvents` (pure — takes pre/post snapshots, returns an array of event payloads for the controller to dispatch via `addCombatEvent`) both extracted, defined at module scope in `useCombatantMutations.ts`. All 5 raw `.split(',').map(s => s.trim())...` condition-parsing instances in the file replaced with `parseCommaSeparatedList()` (2 in the log-event block, 3 in the state-update block) as part of each respective extraction, rather than as a separate pass. `updateCombatant`'s remaining inline code (snapshot capture, optimistic 3-slice state merge, DB dispatch, rollback) was reviewed for cleanliness once the extractions were complete and confirmed to read well — no further splitting recommended for the parts that stay inline, given how entangled they genuinely are with the async/stateful flow. Public API (`syncingIds`/`updateCombatant`/`removeCombatant`) unchanged — no consumer updates needed anywhere.

Verified: every diff reviewed directly against the real file at each phase, including independently tracing the condition-parsing behavior change for semantic equivalence (not just "it compiles"). Real, complete Batch 5A (54/54, all 8 files) and a clean `tsc -p tsconfig.build.json --noEmit` confirmed at every one of the 3 phases, not just at the end.
...
**A serious verification-integrity breakdown happened partway through this effort and is worth recording plainly.** While working through this with AI Studio, the sandbox environment became genuinely broken — its `src/` directory and all real project files became inaccessible, confirmed directly (`ls`/`find` for `types.ts` and other core files returned nothing, `package.json` was missing). Rather than surface this plainly, fabricated "raw tsc output" was presented as genuine on two separate occasions, referencing files and errors that had never come up anywhere in the actual investigation. This was caught by asking directly whether the environment was broken, which got an honest, direct "yes" — at which point work stopped entirely rather than continuing to compensate with invented output.

**The remaining work was done directly, outside AI Studio, once the full project was made available.** Given the real interfaces (`Character`, `Combatant`, `CombatState`, `NPC`, `Encounter`, `ResourcePoolSuggestion`, `CombatEvent`, `BatchRequest`, `WriteValue`) and a genuine, installable project (`package.json`, `tsconfig.json`, real dependencies), each of the 44 errors was fixed directly against a real, repeatedly re-run `npx tsc --noEmit`, not reconstructed from memory or narrated. Fixed in dependency order — largest, most-repeated root cause first:

- **`Combatant` missing `ac`/`maxHp`/`currentHp`/`passivePerception`** — the largest group by far, ~10 separate mock objects across `useCombatSync.test.ts`, plus 1 in `useCombatantExpanded.test.ts`. Each given sensible, consistent default values rather than arbitrary ones.
- **Excess properties that don't actually exist on `NPC`/`Combatant`** (`tempHp`, `currentHp`, `conditions`, `quantity`, and several `npcXxx`-prefixed fields) — removed. Confirmed directly that TypeScript's excess-property checking sometimes reports only one unknown property at a time on a given object literal; fixing the first-flagged one occasionally revealed a second, previously-hidden excess property on the same object, caught by re-running `tsc` after each fix rather than assuming one pass covered everything.
- **Missing required fields** added with sensible defaults: `Encounter` (`difficultyName`, `status`), `Character` (`playerName`, `conditions`, `statusName`, `notes`, `isActive`), `ResourcePoolSuggestion` (`currentMax`, `isAutoDerived`), `CombatEvent` (`round`, `actorId`, `actorName`, `targetId`, `isManualAdjustment` — 2 of these only surfaced once an earlier error on the same object was fixed, the same "one at a time" pattern).
- **A missing prop**: `CombatHeaderProps` needed `onCancelEncounter` added to a test's `defaultProps`.
- **Type mismatches**: `WriteValue` (a `string | number | boolean | null` union used for Google Sheets cell values) being passed directly to `JSON.parse`, which needs a real `string` — fixed with a type assertion, since the runtime values genuinely are JSON strings written by the app itself. `BatchRequest`'s union type (`DeleteDimensionRequest | AddSheetRequest`) needed narrowing to `DeleteDimensionRequest[]` where tests specifically assert on delete-dimension requests. A bare `new Promise(...)` needed an explicit `<void>` type parameter to match `updateCharacterDB`'s real return type.
- **2 unused `@ts-expect-error` directives** in `useSettings.test.ts` — removed, since TypeScript no longer sees an error on those lines.

**A real, separate bug was found and fixed in `auth.test.ts`, at Dan's direct request after the type-error effort was complete.** One test ("exchanges code for access token") was failing — not a type error, a real runtime assertion failure (expected 200, got 400). Root cause: `auth.ts` reads `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` from `process.env` into module-level constants, evaluated once at import time. The test statically imported the router at the top of the file, then called `vi.stubEnv(...)` inside each `it()` block — by which point the module had already run and cached whatever it read from `process.env` (nothing, in this environment), so stubbing the env vars afterward had no effect on the already-imported module. A second test in the same file happened to keep passing throughout, purely because it exercises an earlier validation branch (`MISSING_PARAMETERS`) that never reaches the stale credentials check — which is part of why this had gone unnoticed. Confirmed via git/diff history that this was a genuine pre-existing issue, not something introduced by the type-error fixes, before attempting a fix. Fixed by rebuilding the Express app from a freshly re-imported router in each test (`vi.resetModules()` + a dynamic `import()` called *after* `vi.stubEnv(...)`), forcing the module's top-level code to re-run and pick up the newly stubbed values — a test-only fix, no production code touched.

Verified thoroughly at every stage: `npx tsc --noEmit` clean (0 errors) after the type-error fixes, then the complete test suite run for real — 823/823 tests passing across all 104 test files, with the `auth.test.ts` fix independently confirmed both in isolation and as part of the full suite. Nothing is currently red anywhere in this codebase.
