...
**Incapacitation-breaks-concentration automation doesn't cascade to the concentration effect itself.** Found while fixing a related Hasted/Concentrating bug (now resolved — see `CHANGELOG.md`). `ConditionChips.tsx`'s `addChip()` has existing automation that removes "Concentrating" when an incapacitating condition is added, but it doesn't also remove the associated concentration-requiring effect (e.g. "Hasted") the way the manual-removal cascade now correctly does. Same bug pattern, different trigger path (automatic incapacitation vs. manual chip removal). Not yet fixed — low priority given the surrounding fix is fresh and this is a narrower edge case, but worth closing out for full consistency.
