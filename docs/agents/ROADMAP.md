...
- **`src/components/EncountersTab.tsx` violates the component→services layer boundary — verified, real, confirmed via raw file inspection.** Line 11: `import { readEncounterLogs } from '../services/dbOperations';`. Line 37, inside a `useEffect`: `const logs = await readEncounterLogs();` (used to build `completedEncounterIds`). This directly violates the project's architecture rule `lib ← services ← hooks ← components` — components must never import from `services/` directly; this data-fetching call belongs inside `src/components/EncountersTab/hooks/useEncounters.ts`, following the exact same pattern already used by every other top-level tab component in this app (`CombatantCard.tsx`, `EncounterCard.tsx`, etc. all get their data via a hook, never a direct services import).
  - **Fix**: move the `readEncounterLogs()` call and the `completedEncounterIds` derivation into `useEncounters.ts`, expose the result from that hook, and have `EncountersTab.tsx` consume it from there instead of importing `dbOperations` directly.
  - **Verification requirement before accepting the fix**: raw `tsc` output plus the real batch(es) covering `EncountersTab.tsx` and `useEncounters.ts` (Batch 6B). No summary claims — this project has a long, repeated history of fabricated/narrated batch results; every fix in this backlog must be verified with pasted, literal terminal output before being marked complete.

- **`NewPlayerDialog.tsx`'s Stat Block tab: adding several Actions/Reactions pushes the dialog's footer off-screen with no way to scroll to it — root cause confirmed via direct code inspection.** `DialogShell.tsx`'s outer container has no height constraint; `NewPlayerDialog.tsx`'s own `<form>` wrapper independently caps itself at `max-h-[90vh]` with its own `overflow-y-auto` — meaning the outer shell can still grow to (header + subheader + padding + up to 90vh of form content + footer), exceeding the viewport and pushing the pinned header/footer off-screen.
  - **Proposed fix** (per investigation): constrain `DialogShell.tsx`'s outer container to a real viewport-relative max height, move the scroll/overflow responsibility into `DialogShell`'s own content wrapper, and remove the now-redundant `max-h-[90vh]`/`overflow-y-auto` from `NewPlayerDialog.tsx`'s `<form>`.
  - **⚠️ Critical constraint before implementing**: `DialogShell.tsx` is shared across all 12 dialogs in the app (`CasterAttributionDialog`, `NewEncounterDialog`, `LongRestDialog`, `ShortRestDialog`, `NewNpcDialog`, `NewPlayerDialog`, `AddCombatantDialog`, `ShortcutCheatSheet`, `EncounterLogModal`, `ReferenceDetailDialog`, `LevelUpDialog`, `Soundboard`'s assignment modal) — see `CHANGELOG.md`'s "Modal/Dialog Shell Consolidation" entry. A structural change to `DialogShell.tsx` risks affecting scroll/height behavior in all 12, not just `NewPlayerDialog.tsx`. This must be verified across every consuming batch, not just Batch 6A — confirm the full list of test batches touching any of these 12 dialogs before implementing, and re-run all of them, with raw output, after the fix.
  - **Verification requirement**: raw `tsc` output + raw output for every batch covering any of the 12 `DialogShell` consumers. No summary claims, no partial verification.
