...
- `NpcStatBlockTab.tsx` — Extracted from `NpcFormFields.tsx`. The 4 `NpcListEditor` instances (traits, actions, reactions, legendary actions), delegating to `NpcSimpleFieldEditor.tsx`/`NpcCombatActionFields.tsx` (the same shared components `NpcCard.tsx` uses) rather than the now-deleted `NpcActionEditors.tsx` — see `CHANGELOG.md`.
