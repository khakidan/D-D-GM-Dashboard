...
- `CombatantCard.tsx` — Pure prop-driven card; receives `combatStarted`, `pcCharacter`, `npcModel`, `isActiveTurn`, `isSelected`, `isSelectable`, `isSyncing`, and the `handleResourcePoolUpdate`/`handleConditionAdded`/`handleConditionWithTimer`/`handleExhaustionDeath` functions as props (no direct store access at all — the leaf-component store-access exception previously documented for this component's own hook calls has been fully closed, see `patterns.md`). Wrapped in `React.memo` with a custom comparator that explicitly compares all 4 boolean props alongside its other data props — see `patterns.md`'s "Card memoization pattern."
- `CombatantCardHeader.tsx` — Compact `[-] N/M [+]` resource counter row for every PC resource pool in collapsed view. Passes through a resolved `pcCharacter` prop to `CombatantCompactResourceRow`. Receives `isActiveTurn`/`isSelected`/`isSelectable`/`isSyncing` as props from `CombatantCard.tsx`, not from a store hook.
- `CombatantCardExpanded.tsx` — Full `ResourcePoolsSection` for player combatants. Pure prop-driven (no store access); receives the 4 `handleX` mutation functions as props, called with the specific combatant as their first argument.
