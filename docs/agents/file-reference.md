...
- `conditionDefinitions.ts` — `buildConditionSummary()` and `applyLongRestToConditions()` logic. Imports `CONDITION_MECHANICS` from `conditionMechanicsData.ts`.
- `conditionMechanicsData.ts` — `ConditionMechanics` interface and the 500-line `CONDITION_MECHANICS` constant.
...
- `abilityFundamentals.ts` — `abilitiesInOrder`, `AbilityName`, `calculateModifier()`, `proficiencyBonusFromLevel()`. Extracted from `abilityScores.ts` specifically to break a circular import with `spellcasting.ts` — see `CHANGELOG.md`. `abilityScores.ts` re-exports all 4 symbols, so this split is transparent to every existing consumer; import from `abilityScores.ts` as before unless working inside `abilityScores.ts`/`spellcasting.ts` themselves.
- `abilityScores.ts` — `AbilityScores` and `Proficiencies` types, `proficiencyBonusFromCR()`, skill/save helpers, parse/serialize helpers. Re-exports `abilitiesInOrder`/`AbilityName`/`calculateModifier`/`proficiencyBonusFromLevel` from `abilityFundamentals.ts` (see above). `Proficiencies` includes optional `spellcastingAbility?: SpellcastingAbility` for GM override of auto-derived caster stat and optional `toughFeat?: boolean` (defaulting to `false` in `DEFAULT_PROFICIENCIES`).
...
- `dbOperations/` — Folder containing decomposed sheet operation modules.
  ...
  - `encounterCombatants.ts` — Handles adding, updating, and deleting encounter combatant records. `addEncounterCombatantDB` generates each new row's ID via `generateUuid()` (not `getNextId`) — deliberately, since concurrent calls (e.g. `useEncounterLifecycle.ts`'s unawaited per-PC add loop at combat start) previously raced against `getNextId`'s scan-then-increment scheme and could be assigned duplicate IDs — see `CHANGELOG.md`.
...
- `campaigns.ts` — `POST /api/campaigns/create` provisions all 7 sheets with correct headers (Characters, NPCs, Encounters, Encounter_Combatants, Status, Difficulty_Level, EncounterLogs — `Conditions`/`Spells` are seeded separately, later, via `ReferenceDataSeeder.tsx`; see `schema.md`). Imports `CHARACTER_HEADERS`/`NPC_HEADERS`/`ENCOUNTER_HEADERS`/`ENCOUNTER_COMBATANT_HEADERS`/`ENCOUNTER_LOG_HEADERS` from `sheetSchemas.ts` as the single source of truth for column order, shared with the Zod parsing schemas — see `CHANGELOG.md`.
- `auth.ts` — Proxies OAuth token exchanges (keeps secrets off the client). `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` are read from `process.env` into module-level constants at import time — a real gotcha for tests, since stubbing env vars after the module has already been imported has no effect (see `CHANGELOG.md`).
- `health.ts` — Health check endpoint.
- `rateLimiter.ts` — `createRateLimiter(message)` factory; returns a structured `{ error: 'TOO_MANY_REQUESTS', message }` JSON response on rate-limit trip.
- `bodyValidation.ts` — `requireBody` middleware, rejects requests with an empty/missing JSON body.
- `utils/errors.ts` — `sendError(res, status, code, message, extra?)`. Standardizes server error response shape (`{ error, message, ...extra }`) across `campaigns.ts` and `auth.ts` — see `CHANGELOG.md`.
...
### src/components/ui/ (shared components, decomposed into subfolders)

#### src/components/ui/combat/
...
- `ConditionSearchDropdown.tsx` — Presentational search/add dropdown component extracted from `ConditionChips.tsx`.
- `ConditionDurationPrompt.tsx` — Presentational duration prompting component extracted from `ConditionChips.tsx`.
...
#### src/components/ui/card-primitives/
...
- `StatTile.tsx` — Pure shell component for the bordered label-over-value tile pattern (`AC`, `Max HP`, `HP`, `CR`, ability scores, etc.). Does not own input behavior — accepts the value content (static text, `CardNumberInput`, `DebouncedInput`, or `AbilityScoreInput`) as `children`, matching the `DialogShell` "shell owns chrome, not content" philosophy. Supports an optional third-row `modifier` (color-coded, formatBonus is imported from `src/lib/stringUtils.ts`) and `size?: 'default' | 'compact'`. Adopted in `NpcCard.tsx` (`AC`/`Max HP`/`CR`), `CombatantCardExpanded.tsx` (`Temp HP`/`Max HP`, editable), `CharacterCardExpanded.tsx` (`AC`/`Max HP`/`HP`/`Temp`/`Level` — a leftover gap from the original `StatTile` build, easily confused with `CombatantCardExpanded.tsx`, found and fixed later — see `CHANGELOG.md`), and `StatBlockScoresTable.tsx` (all six ability scores, `size="compact"`, the adoption that validates the `modifier` row in a rendered instance).
...
- `SettingsPanel.tsx` — Minimal shared wrapper for the Settings page's card containers (`bg-white p-6 rounded-2xl border border-[#e2e8f0] shadow-sm space-y-4`, previously duplicated verbatim across 5 files). Takes `children`, `className` (merged via `cn()`), and forwards all other `<div>` props. Adopted in `SheetConnectionSettings.tsx`, `AuthPortalSettings.tsx`, `GMTestingTools.tsx`, `ReferenceDataSeeder.tsx`, and inline in `SettingsPage.tsx` for the Backup card — see `CHANGELOG.md`.
...
- `markdownComponents.tsx` — Shared `baseMarkdownComponents` mapping for `react-markdown`. Contains the standard global overrides (tables, lists, text styling) shared between `ReferenceDetailDialog` and `NpcStatBlockSection` to maintain visual consistency without duplicating the base configuration object.
- `MarkdownRenderer.tsx` — Public lazy-loading orchestrator for markdown content. Uses `React.lazy()` and `<Suspense>` in production, with dedicated synchronous test mock support, custom fallbacks, and markdown component customization.
- `LazyMarkdown.tsx` — Dynamic lazy-loaded target wrapping the heavy imports of `react-markdown` and `remark-gfm` to isolate the markdown parsing and rendering dependency stack.
...
- `DashboardLayout.tsx` — Layout wrapper for the main dashboard views. Includes a header with `title`, `description`, and an `actions` slot, plus optional `filterControls` and a scrollable `children` area.
- `SectionHeader.tsx` — Shared uppercase `h3` with `border-b` and `tracking-widest`. Supports `size="compact"` (text-[10px]) or `"default"` (text-xs). Used for consistent sub-section labeling.


#### src/components/ui/stat-block/

- `StatBlock.tsx` — Orchestrator (~116 lines). Delegates rendering to `StatBlockScoresTable.tsx`, `StatBlockSaves.tsx`, `StatBlockPassive.tsx`, and `StatBlockSkills.tsx`.
- `StatBlockScoresTable.tsx` — Horizontal table for ability scores and modifiers. Supports optional editable mode via `onScoreChange` prop (used by `StatBlock` and expanded combatant cards).
- `StatBlockSaves.tsx` — Grid display for six ability saving throw proficiencies and calculated bonuses with click-to-toggle support via `onToggle`.
- `StatBlockPassive.tsx` — Displays passive Perception, Insight, and Investigation scores, with expandable numeric inputs for feat/item passive bonuses via `onPassiveBonusChange`.
- `StatBlockSkills.tsx` — Collapsible skill list with proficiency/expertise toggles, ability score grouping, calculated bonuses, and Jack of All Trades support.
- `NpcStatBlockSection.tsx` — Pure display component for one NPC stat block section (traits, actions, reactions, bonus actions, or legendary actions). Accepts a `title` and `items[]` with optional metadata. Also exports `formatActionMeta(action)` to build a compact mechanical summary string from `NpcAction` fields (`attackBonus`, `damage`, `saveDC`, `saveType`, `range`, `recharge`). Returns `null` when empty.

#### src/components/ui/npc-editor/

- `NpcFormFields.tsx` — Shared form orchestrator used by both `NewNpcDialog` and the `AddCombatantDialog` Create NPC tab. Must stay in sync between both usages. Fully decomposed (~140 lines) — delegates each of its 4 tabs to a dedicated component (`NpcIdentityTab.tsx`, `NpcCombatTab.tsx`, `NpcAbilitiesTab.tsx`, `NpcStatBlockTab.tsx`, all in `src/components/ui/npc-editor/`) while keeping the external API (`data`, `onChange`, `errors`, `compact`) unchanged — see `CHANGELOG.md` for the full extraction history. `rechargeAbilities` has been removed from `NpcFormData`, `DEFAULT_NPC_FORM_DATA`, and associated handlers. Recharge is now represented only by the `recharge` field on `NpcAction`.
- `NpcIdentityTab.tsx` — Extracted from `NpcFormFields.tsx`. Name, CR, speed, senses, languages fields.
- `NpcCombatTab.tsx` — Extracted from `NpcFormFields.tsx`. AC, HP, IRV, legendary actions/resistances, notes fields.
- `NpcAbilitiesTab.tsx` — Extracted from `NpcFormFields.tsx`. Wraps `StatBlock` for ability score/proficiency editing; handles the string↔object serialization `NpcFormData` requires (unlike `NewPlayerDialog.tsx`'s equivalent tab, which holds these as live objects and only serializes on confirm — a genuine, load-bearing data-shape difference, not just a styling one; see `CHANGELOG.md` for the investigation that ruled out sharing a component between the two).
- `NpcStatBlockTab.tsx` — Extracted from `NpcFormFields.tsx`. The 5 `NpcListEditor` instances (traits, actions, reactions, bonus actions, legendary actions), delegating to `NpcSimpleFieldEditor.tsx`/`NpcCombatActionFields.tsx` (the same shared components `NpcCard.tsx` uses) via the shared renderer factory `npcListFieldRenderers.tsx` — see `CHANGELOG.md`.
- `npcListFieldRenderers.tsx` — Shared render-prop factory for the 5 NPC/PC list editors (traits, actions, reactions, bonus actions, legendary actions) designed to consolidate bytes-identical code and keep layout rendering logic highly unified across `CharacterCardExpanded.tsx`, `NewPlayerDialog.tsx`, `NpcCard.tsx`, and `NpcStatBlockTab.tsx`.
- `NpcListEditor.tsx` — Generic list editor for NPC/PC arrays (Traits, Actions, etc). Supports collapsible headers with `defaultExpanded` control. The Add action uses `Button.tsx` (`tertiary` intent with explicit padding for a larger hit-target) and the Remove action uses `IconButton.tsx` (`destructive` intent), both with `stopPropagation` protection.
...
- `hooks/usePartyLevelUp.ts` — Extracted from `useParty.ts`. Contains `levelUpCharacter`/`handleLevelUp`/`cancelLevelUp`/`handleLevelUpConfirm` state and handlers. Receives `mirrorCharacterFieldsToCombatants` via dependency injection from `useParty.ts` rather than duplicating it.
- `hooks/usePartyCharacterCrud.ts` — Extracted from `useParty.ts`. Contains `handleCreateCharacter`/`handleDeletePlayer`/`handleUpdate` — the last of which is the field whitelist described in `schema.md`, and where the concentration-check-on-damage logic described in `patterns.md` actually lives. Also receives `mirrorCharacterFieldsToCombatants` via dependency injection.
...
- `index.tsx` — Top-level coordinator. Owns `characters`/`npcs` derivation for combatants (resolves `pcCharacter`/`npcModel` once per combatant in its `.map()`, passing resolved data down), plus `isActiveTurn`/`isSelected`/`isSelectable`/`isSyncing` (once per combatant) and the shared `useCombatantExpanded()` mutation functions — resolved once here, not inside `CombatantCard.tsx` — see `CHANGELOG.md`.
...
- `CombatantCard.tsx` — Pure prop-driven card; receives `combatStarted`, `pcCharacter`, `npcModel`, `isActiveTurn`, `isSelected`, `isSelectable`, `isSyncing`, and the `handleResourcePoolUpdate`/`handleConditionAdded`/`handleConditionWithTimer`/`handleExhaustionDeath` functions as props (no direct store access at all — the leaf-component store-access exception previously documented for this component's own hook calls has been fully closed, see `patterns.md`). Unified expand panel: the previous standalone "▶ Stat Block" toggle has been removed, and all reference content is now rendered directly within the main expanded panel. Wrapped in `React.memo` with a custom comparator that explicitly compares all 4 boolean props alongside its other data props — see `patterns.md`'s "Card memoization pattern."
- `CombatantCardHeader.tsx` — Compact `[-] N/M [+]` resource counter row for every PC resource pool in collapsed view. Passes through a resolved `pcCharacter` prop to `CombatantCompactResourceRow.tsx`. Receives `isActiveTurn`/`isSelected`/`isSelectable`/`isSyncing` as props from `CombatantCard.tsx`, not from a store hook.
- `CombatantCardExpanded.tsx` — Expanded content orchestrator for both PC and NPC combatants. Follows standard 5e stat-block flow: renders Speed, CR, Proficiency, Ability Scores (using `StatBlockScoresTable`), Saves, Passives, Senses/Languages (NPC-only), Skills, IRVs, Resource Pools (for PCs), Recharge Tracker (for NPCs), Mechanics Summary, and reference content (Traits, Actions, Bonus Actions, Reactions, Legendary Actions). Pure prop-driven (no store access); receives mutation functions as props.
...
- `hooks/useCombatLifecycle.ts` — Extracted from `useCombatSync`. Contains `resetCombat`, `cancelCombat`, `rollInitForNPCs`, and `handleCallInitiative`. Handles initiative rolling for NPCs, resetting and canceling active combat encounters with database persistence and rollback, and combat log start/end emissions. The module-level `ensureCombatLogInitialized` helper builds its combatant snapshot via the shared `buildPartySnapshot()` (in `lib/combatLog.ts`), also used by `useEncounterResume.ts`'s resume-time log reconstruction — see `CHANGELOG.md`/`ROADMAP.md`.
