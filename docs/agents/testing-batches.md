...
**Current baseline: 1013 tests.** Real, verified totals for every affected batch, run individually per this file's own rule:
...
- **Batch 6A: 59 → 61.** Breakdown: 59 (baseline) + 1 (new test covering Max Uses / Reset edits inside CharacterCardExpanded) + 1 (new test covering Max Uses / Reset edits inside NewPlayerDialog) = 61.
...
- **Batch 8: 68 → 73.** Breakdown: 68 (baseline) + 5 (1 new test in AbilitySelectChips, plus 4 new tests in NpcCombatActionFields covering Atk automation) = 73.
...
| 6A | `src/components/PartyTab/__tests__` | 61 |
...
| 8 | `src/components/ui/__tests__` | 73 |
