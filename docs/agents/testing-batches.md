...
**Current baseline: 995 tests.** Real, verified totals for every affected batch, run individually per this file's own rule:
...
- **Batch 5B: 56 → 57.** Breakdown: 56 (baseline) + 1 (new test confirming Reactions with mechanical fields meta string in expanded layout) = 57.
...
- **Batch 6C: 27 → 28.** Breakdown: 27 (baseline) + 1 (new test covering Reaction added with mechanical fields in NewNpcDialog) = 28.
...
- **Batch 8: 61 → 62.** Breakdown: 61 (baseline) + 1 (new test confirming legacy vs mechanical reaction parsing via formatActionMeta) = 62.
