...
**Current baseline: 972 tests.** Real, verified totals for every affected batch, run individually per this file's own rule:
- **Batch 1: 488 → 492.** Breakdown: 488 (baseline) + 3 (sheetSyncParser.test.ts null/undefined row handling ×2, combatantBuilder.test.ts GM-controlled character mapping ×1) + 1 (untracked pre-existing test in combatLogic.test.ts, cause unconfirmed — flagged as genuine baseline drift, not attributed to any specific prior change) = 492.
...
- **Batch 6C: 24 tests.** Not touched this session.
...
- **Batch 8: 51.** Not touched this session.
