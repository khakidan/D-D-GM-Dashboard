...
1. NPC Library expanded view via `NpcStatBlockSection` (`src/components/ui/stat-block/`)
2. Active Encounter NPC cards via `CombatantCardExpanded.tsx` — the standalone `NpcReferencePanel.tsx`/`PcReferencePanel.tsx` components were deleted and their functionality unified into this single component (see `CHANGELOG.md`'s "Unified Expanded Layout" entry)
...
Import it from:
```ts
src/components/ui/stat-block/NpcStatBlockSection
```
...
`AbilityScoreInput` is a local-state wrapper, defined and used only internally within `StatBlockScoresTable.tsx` (`src/components/ui/stat-block/`) — not a separately exported or reusable component. It keeps the raw typed value locally and commits changes only on blur or Enter. This prevents mid-keystroke snap-to-default behavior.
...
### Card memoization pattern (`CharacterCard`/`NpcCard`/`EncounterCard`/`CombatantCard`)

All 4 main entity cards are wrapped in `React.memo` with a custom comparator function, not the default shallow-prop comparison — see `CHANGELOG.md`'s performance-fix entry for the full investigation. The comparator explicitly lists each *data* prop (the entity object itself, plus primitives like `isSyncing`/`isExpanded`) and deliberately omits every callback prop (`onUpdate`, `onDelete`, `onToggleExpand`, etc.), since each tab's `.map()` call site creates fresh callback references on every render, but they always represent the same underlying operation for that entity.

This only works because every relevant state-update path uses the pattern `.map(item => matches ? {...item, ...updates} : item)` — which preserves the exact same object reference for every item that wasn't actually changed, even after a subsequent array re-sort (sorting reorders the array without cloning its elements). **If a future mutation path stops following this pattern** (e.g. rebuilding the whole array with `.filter().map()` chains that don't return the original reference, or mapping through an intermediate transform that clones every item), these comparators would silently stop skipping unnecessary re-renders — not a crash, just a quiet loss of the performance benefit. When adding a new mutation path for characters/NPCs/encounters/combatants, keep the reference-preserving `.map()` pattern.

**When adding a new prop to any of these 4 card components**, add it to that component's comparator too (in the file itself, at the bottom). A prop left out of the comparator doesn't cause a build error or a test failure by itself — it just means changes to that prop get silently ignored by the memo, which shows up as a real bug (a card not updating when it should) rather than a performance problem. This is a manual-sync burden with no automatic enforcement; worth checking in review whenever one of these 4 components' props interfaces change. `CombatantCard.tsx`'s comparator is the most involved of the four, since `isActiveTurn`/`isSelected`/`isSelectable`/`isSyncing` are real props there (not derived internally via a hook) — see the "Store-access architecture" section below for why that matters specifically for this component.
...
**Formerly-documented exception, now resolved.** `useCombatantCard.ts` and `useCombatantExpanded.ts` previously called `useDashboardStore()` directly from inside `CombatantCard.tsx` (a leaf component) — a deliberate, temporary performance tradeoff at the time (see `CHANGELOG.md`'s original performance-fix entry), rather than an oversight. That exception has since been fully closed: `useCombatantCard.ts` was deleted entirely (its four real outputs — `isActiveTurn`, `isSelected`, `isSelectable`, `isSyncing` — are now derived once in `ActiveEncounterTab/index.tsx` and passed down as props, the same way `pcCharacter`/`npcModel` already worked), and `useCombatantExpanded.ts` was redesigned to be called once at the coordinator (`useCombatantExpanded()`, no combatant argument) with each returned mutation function taking the specific `Combatant` as its first parameter at the call site, rather than the hook closing over one combatant per card. See `CHANGELOG.md` for the full fix. `CombatantCard.tsx`'s custom `React.memo` comparator now explicitly compares these 4 booleans as real props (see the "Card memoization pattern" section above) — this is the one place a future prop addition to this component most needs to remember the comparator-sync rule, since these specific fields used to be invisible to the comparator entirely before this fix.
