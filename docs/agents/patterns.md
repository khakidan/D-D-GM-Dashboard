...
**Documented exception: `useCombatantCard.ts` and `useCombatantExpanded.ts`.** Both are hooks called from inside `CombatantCard.tsx`, a leaf component, and both call `useDashboardStore()` directly rather than receiving their data as props — on the surface, a violation of the rule above. This was a deliberate performance tradeoff, not an oversight (see `CHANGELOG.md`'s performance-fix entry for the full story): both hooks previously called `useAppState()`, whose broad, whole-app-state subscription meant any combatant's update re-rendered every `CombatantCard` regardless of props, making `React.memo` on the outer component pointless. The fix narrows each hook's subscription (a per-combatant Zustand selector with `useShallow` for `useCombatantCard`, and a subscription-free `getSnapshot` plus a narrow `updateState` selector for `useCombatantExpanded`) rather than eliminating direct store access entirely. The fully-consistent alternative — resolving `isActiveTurn`/`isSelected`/`isSyncing` at `ActiveEncounterTab/index.tsx` (all 3 are already available there via existing hook calls) and passing them down as props, the same way `pcCharacter`/`npcModel` already work — was considered and would close this gap completely, but is a larger refactor than a performance-focused change warranted; see `ROADMAP.md` if that consistency becomes worth the extra risk later. Don't treat this exception as precedent for other leaf components reaching into the store for convenience — it was earned by a specific, verified performance problem, not a shortcut.
