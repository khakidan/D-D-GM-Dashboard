...
describe('sheetSchemas', () => {
  ...
  it('CharacterRowSchema pads results to CHARACTER_HEADERS.length columns', () => {
    const row = ['char-1', 'Player', 'Hero'];
    const result = CharacterRowSchema.parse(row);
    expect(result).toHaveLength(CHARACTER_HEADERS.length);
  });

  it('NpcRowSchema pads results to NPC_HEADERS.length columns', () => {
    const row = ['npc-1', 'Goblin'];
    const result = NpcRowSchema.parse(row);
    expect(result).toHaveLength(NPC_HEADERS.length);
  });

  it('EncounterRowSchema pads results to ENCOUNTER_HEADERS.length columns', () => {
    const row = ['enc-1', 'Ambush'];
    const result = EncounterRowSchema.parse(row);
    expect(result).toHaveLength(ENCOUNTER_HEADERS.length);
  });

  it('EncounterCombatantRowSchema pads results to ENCOUNTER_COMBATANT_HEADERS.length columns', () => {
    const row = ['ec-1', 'enc-1'];
    const result = EncounterCombatantRowSchema.parse(row);
    expect(result).toHaveLength(ENCOUNTER_COMBATANT_HEADERS.length);
  });
});
