...
describe('NewPlayerDialog', () => {
  ...
  it('allows editing Max Uses and Reset for Actions in GM-controlled PC context inside NewPlayerDialog', () => {
    const onConfirmMock = vi.fn();
    ...
    // Fill in required fields on Identity tab
    fireEvent.change(
      getByPlaceholderText('e.g. Sarah'),
      { target: { value: 'Tracker User' } }
    );
    fireEvent.change(
      getByPlaceholderText('e.g. Drogar'),
      { target: { value: 'Tracked Char' } }
    );

    // Navigate to Stat Block tab
    fireEvent.click(getByRole('tab', { name: /stat block/i }));

    // Click "GM-Controlled Character" checkbox
    const gmCheckbox = screen.getByLabelText('GM-Controlled Character');
    fireEvent.click(gmCheckbox);
    ...
    // Fill in name
    fireEvent.change(getByPlaceholderText('Action name (e.g. Bite)'), { target: { value: 'Smite' } });

    // Find and fill Max Uses
    const maxUsesInput = screen.getByLabelText('Max Uses');
    fireEvent.change(maxUsesInput, { target: { value: '4' } });

    // Find and fill Reset condition
