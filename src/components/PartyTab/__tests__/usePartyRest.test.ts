...
describe('useParty - REST and Recovery', () => {
  ...
  describe('Rest Selection and Pool Logic', () => {
    ...
    it('mirrors short rest HP updates to active PC combatant', async () => {
      const updateStateSpy = vi.fn();
      const char = { 
        id: 'pc-1', 
        currentHp: 10, 
      ...
      await act(async () => {
        await result.current.handleShortRest([
          { characterId: 'pc-1', hpToAdd: 15, newHitDiceUsed: '{}' }
        ]);
      });
      ...
      // Verify character sheet has been updated
      expect(nextState.characters[0].currentHp).toBe(25);

      // Verify active combatant has mirrored the changes
      const updatedCombatant = nextState.combatState.combatants[0];
      expect(updatedCombatant.currentHp).toBe(25);
      expect(updatedCombatant.maxHp).toBe(40); // remains stable
    });
