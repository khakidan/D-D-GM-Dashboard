...
export function DebouncedTextarea({ value, onChange, onBlur, className, ...props }: DebouncedTextareaProps) {
  ...
  return (
    <textarea
      ...
      className={cn(
        "bg-white border border-[#e2e8f0] rounded-xl text-[#0f172a] placeholder:text-[#8d8db9] focus:border-[#2563eb] focus:ring-1 focus:ring-[#2563eb]/50 focus:outline-none w-full px-3 py-2 text-sm resize-y transition-colors",
        className
      )}
      {...props}
    />
