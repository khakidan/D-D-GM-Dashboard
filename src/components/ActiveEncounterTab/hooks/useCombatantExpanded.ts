...
export function useCombatantExpanded(c: Combatant) {
  ...
  const handleConditionWithTimer = async (
    ...
    let currentCondList = parseCommaSeparatedList(c.conditions);
        ...
        combatants: prev.combatState.combatants.map((combatant) =>
          combatant.id === c.id
            ? {
                ...
                statusId: 3,
                isStable: false,
                conditions: parseCommaSeparatedList(combatant.conditions)
                  .filter((cond) => cond.toLowerCase() !== 'unconscious')
                  .join(', '),
              }
            : combatant
        ),
